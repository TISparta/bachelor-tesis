...
Seguidamente, sea $\lambda$ la longitud de onda de la entrada y $p$ los parámetros que caracterizan al
diseño, denotaremos como $T_{\lambda}(p)$ a la transmitancia asociada al dispositivo obtenido con la
parametrización $p$ en la longitud de onda $\lambda$. Luego, definimos la función objetivo 
...
\begin{equation}
  f_{obj}(p) = max \left \{ T_{1550} (p) \right \}.
\label{eq:fom-bend}
\end{equation}
...
Pero, en este caso tenemos dos guías de salida, por ello, usaremos como notación $T_{\lambda}^{(1)}(p)$ para
representar la transmitancia en la guía de salida superior cuando se recibe un haz de longitud de onda
$\lambda$ en un diseño descrito por los parámetros $p$ y $T_{\lambda}^{(2)}(p)$ para la guía de salida inferior.
...
de $2.0\mu m \times 2.0 \mu m$. En el lado izquierdo se muestra la representación de $|E|^2$ cuando se usa 
...
de salida. El diseño intuitivo para este dispositivo es el presentado en la imagen. Similar al caso del
...
\begin{equation}
  f_{obj}(p) = max \left \{ \left ( T_{1300}^{(1)}(p) \right )^2  + 
                            \left ( 1 - T_{1300}^{(2)}(p) \right )^2 +
                            \left ( 1 - T_{1550}^{(1)}(p) \right )^2  + 
                            \left ( T_{1550}^{(2)}(p) \right )^2 
                    \right \}.
